...
public interface IInmueblesService
{
    ...
    Task<InmuebleResponseDTO> GetByIdAsync(Guid inmuebleId, Guid? ownerId, string? role);
    Task<List<InmuebleListResponseDTO>> ListAsync(InmuebleFilterDTO? filter, Guid? ownerId, string? role);
